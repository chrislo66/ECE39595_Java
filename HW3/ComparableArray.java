...
\   * return -1 if this < a, 0 if this equals a, 1 if this > a
   ...
   public int compareTo(ComparableArray a) {
    int compare = 0;
    int min_ary = min(this.getLength(), a.getLength()); 
    
    //Compare logic
    if (this.getLength() == a.getLength()) {
      for (int k = 0; (compare == 0) && (k < a.getLength()); k++) {
        if(this.getElement(k) > a.getElement(k)) compare = 1;
        else if(this.getElement(k) < a.getElement(k)) compare = -1;
        else if(this.getElement(k) == a.getElement(k)) compare = 0;
      }
    }
    else if (this.getLength() != a.getLength()) {
      for (int k = 0; (compare == 0) && (k < min_ary); k++) { //compare the prefix of two arrays first: until k=the lenght of shorter array
        if(this.getElement(k) > a.getElement(k)) compare = 1;
        else if(this.getElement(k) < a.getElement(k)) compare = -1;
        else if(this.getElement(k) == a.getElement(k)) compare = 0;
      }
      ...
      if (compare == 0) {
        if ((this.getLength() - a.getLength()) < 0) {compare = trailingNonZero(this, min_ary);}
         else if ((a.getLength() - this.getLength()) < 0) {compare = trailingNonZero(a, min_ary);}
      }
    }
    return compare;
   }
   ...
   // I used this for compareTo, for cases 5 and 6 (a = longer array; i = length of shorter array)
